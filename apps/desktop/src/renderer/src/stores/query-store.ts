...
function generateSampleValue(column: Column, rowIndex: number): unknown {
  ...
  if (lower.includes('numeric') || lower.includes('decimal') || lower.includes('float') || lower.includes('double')) {
...
const sampleHistory: QueryHistoryItem[] = [
  {
    id: '1',
    query: "SELECT * FROM users WHERE created_at > now() - interval '7 days' ORDER BY created_at DESC LIMIT 100",
